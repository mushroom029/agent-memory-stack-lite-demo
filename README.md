...
- 完整 session-log.md 继续保存在本地，不强制删除、轮转、压缩或改写。
- 正常恢复只读最近 30-60 行或几张审计卡；旧证据只在锚点指向、冲突或信息缺失时定向检索/读行段。
- ctive-task.md 只保留当前路线、关键纠正/否决、稳定边界、证据指针和下一步，不逐批累计时间线。
- 健康检查只给软警告；按索引选中的相关 capsule 不做总量硬砍。
- 保留 v0.2.4 的任务提示文案、一个公开 skill 和已有工程协议时退让。

普通用户不需要理解内部机制。安装后在真实项目会话里说一句 启用外挂记忆 即可。

## 下载

- [v0.2.5 最新版](./agent-memory-stack-lite-demo-v0.2.5.zip)
...
`	ext
v0.2.5 E20A8D3A47EC55B7D7A6843715B1B835F41FCADAEF31E29820CBBB93E2792209
